...
const char TX_ADDRESS0[TX_ADR_WIDTH]=   {0x01,0x43,0x10,0x10,0x01}; //发送地址
...
char NRF24L01_Read_Buf(char reg,char *pBuf,char len)
{
	char status,char_ctr;	       
 	...
 	for(char_ctr=0;char_ctr<len;char_ctr++)pBuf[char_ctr]=SPI2_ReadWriteByte(0XFF);//读出数据
...
char NRF24L01_Write_Buf(char reg, char *pBuf, char len)
{
	char status,char_ctr;	    
 	NRF24L01_CSN = 0;          //使能SPI传输
  	status = SPI2_ReadWriteByte(reg);//发送寄存器值(位置),并读取状态值
  	for(char_ctr=0; char_ctr<len; char_ctr++)SPI2_ReadWriteByte(*pBuf++); //写入数据	 
  	NRF24L01_CSN = 1;       //关闭SPI传输
  	return status;          //返回读到的状态值
}				   
//启动NRF24L01发送一次数据
