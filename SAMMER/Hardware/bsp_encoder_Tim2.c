...
*						1027259561@qq.com
*
* 文件名称：bsp_encoder_Tim2.h bsp_encoder_Tim2.c
* 文件摘要：
*						这个是STM32定时器编码器模式的应用，应用了TIM2，读取编码器的值
*						
* 当前版本：v1.0
* 当前作者：YJ_李红禧
...
/*
***************************************************
*函数名称：vENCODER_TIM2_NVIC_Config(void)
*
*函数功能：配置定时器的中断优先级  0-0-2
*入口参数：无
*返回值：  无
*
*当前作者：LHX
...
/*
***************************************************
*函数名称：vENCODER_TIM2_GPIO_Config(void)
*
*函数功能：配置GPIO引脚
*入口参数：无
*返回值：  无
...
/*
***************************************************
*函数名称：vENCODER_TIM2_Mode_Config(void)
*
*函数功能：配置定时器的模式
*入口参数：无
*返回值：  无
...
static void vENCODER_TIM2_Mode_Config(void)
{
    ...
    /* 开启编码器模式2，A B相同时计数， 上升下降都计数，因此记得的数为四倍 */
